...
#define R_MAIN_PIN 2  // Rotary Encoder main pin (right)
#define R_SECD_PIN 4  // Rotary Encoder second pin (left)
#define R_BUTN_PIN 3  // Rotary Encoder push button pin
...
#define buzzerPIN 11          // The simple buzzer to make a noise
#define lcdled_PIN 9          // The simple buzzer to make a noise
#define ledPIN 18             // The simple buzzer to make a noise
#define heaterBIT 0b00000100  // The Heater pin, D10, is 2-nd bit on the PORTB register
...
bool IRON::checkSWStatus(void) {
  if (disconnected)  //if IRON is disconnected return
    return;
  bool state = digitalRead(tPIN);
  if (last_tilt_state != state) {
    last_tilt_state = state;
    last_tilt_change = millis();
  }
  if ((tilt_toggle != last_tilt_state) && (millis() - last_tilt_change > 100)) {
    tilt_toggle = last_tilt_state;
    return true;
  }
  return false;
}

bool IRON::isIronTiltSwitch(bool tilt_invert) {
  return tilt_invert ? tilt_toggle : !tilt_toggle;
}
...
#endif
#ifndef DEBUGEEE
  pinMode(ABSORBER_PIN, OUTPUT);
  digitalWrite(ABSORBER_PIN, LOW);
#endif
...
void loop() {
  ...
  if (iron.checkSWStatus() && pCurrentScreen == &wrkScr) {//checks status and returns if changed to reset the timeout
    pCurrentScreen->resetTimeout();
    Serial.println("CD.");
    Serial.println(iron.isIronTiltSwitch(ironCfg.isReedType()));
    
  }
#ifndef DEBUGEEE
  digitalWrite(ABSORBER_PIN, (iron.isOn() && iron.isIronTiltSwitch(ironCfg.isReedType())));
#endif
}
