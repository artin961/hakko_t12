...
#define absorber 1        //TX PIN AS AABSORBER OPTOCOUPLER OUT
...
const uint8_t probePIN  = A1;                   // Thermometer pin from soldering IRON
const uint8_t checkPIN  = A0;                   // Iron current check pin
...
const uint8_t lcdled_PIN = 9;                   // The simple buzzer to make a noise
const uint8_t ledPIN = 18;                   // The simple buzzer to make a noise
...
class BUZZER {
  public:
    ...
    void    failedBeep(void) 			    {
      if (on) {
        tone(buzzer_pin, 3520, 160); delay(170);
...
#endif
#ifndef DEBUGEEE
  pinMode(absorber, OUTPUT);
  digitalWrite(absorber, LOW);
#endif
...
void loop() {
  ...
  iron.checkSWStatus();
  digitalWrite(absorber, (iron.isOn() && iron.isIronTiltSwitch(ironCfg.isReedType())));
